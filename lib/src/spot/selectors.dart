...
extension WidgetMatcherExtensions<W extends Widget> on WidgetMatcher<W> {
  ...
  WidgetMatcher<W> hasProp2<T, R extends RenderObject>({
    required NamedProp<W, R, T> prop,
    required MatchProp<T> match,
  }) {
    final ConditionSubject<Element> conditionSubject = it<Element>();
    final Subject<T> subject = conditionSubject.context.nest<T>(
      ...
      (element) {
        if (prop.renderObjectSelector != null) {
          return Extracted.value(
            prop.renderObjectSelector!.call(element.renderObject! as R),
          );
        }
        if (prop.widgetSelector != null) {
          final widget = selector.mapElementToWidget(element);
          return Extracted.value(prop.widgetSelector!.call(widget));
        }
        assert(prop.elementSelector != null);
        return Extracted.value(prop.elementSelector!.call(element));
      },
    );
...
NamedElementProp<W, R, T>
    elementProp<W extends Widget, R extends RenderObject, T>(
  String name,
  T Function(Element element) prop,
) {
  return NamedElementProp(
    name: name,
    prop: prop,
  );
}

NamedWidgetProp<W, R, T>
    widgetProp<W extends Widget, R extends RenderObject, T>(
  String name,
  T Function(W widget) prop,
) {
  return NamedWidgetProp(
    name: name,
    prop: prop,
  );
}

RenderObjectTypeCurry<R> renderObject<R extends RenderObject>() {
  return RenderObjectTypeCurry<R>();
}

class RenderObjectTypeCurry<R extends RenderObject> {
  RenderObjectTypeCurry();

  NamedRenderObjectProp<W, R, T> prop<W extends Widget, T>(
    String name,
    T Function(R renderObject) prop,
  ) {
    return NamedRenderObjectProp(
      name: name,
      prop: prop,
    );
  }
...
NamedRenderObjectProp<W, R, T>
    renderObjectProp<W extends Widget, R extends RenderObject, T>(
  String name,
  T Function(R renderObject) prop,
) {
  return NamedRenderObjectProp(
...
class NamedRenderObjectProp<W extends Widget, R extends RenderObject, T>
    extends NamedProp<W, R, T> {
  NamedRenderObjectProp({
    required super.name,
    required T Function(R renderObject) prop,
  }) : super(renderObjectSelector: prop);
}

class NamedWidgetProp<W extends Widget, R extends RenderObject, T>
    extends NamedProp<W, R, T> {
  NamedWidgetProp({
    required super.name,
    required T Function(W widget) prop,
  }) : super(widgetSelector: prop);
}

class NamedElementProp<W extends Widget, R extends RenderObject, T>
    extends NamedProp<W, R, T> {
  NamedElementProp({
    required super.name,
    required T Function(Element widget) prop,
  }) : super(elementSelector: prop);
}

class NamedProp<W extends Widget, R extends RenderObject, T> {
  final String name;
  final T Function(Element element)? elementSelector;
  final T Function(W widget)? widgetSelector;
  final T Function(R renderObject)? renderObjectSelector;
